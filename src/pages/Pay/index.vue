  ...
  methods: {
    ...
    async open() {
      ...
      this.$alert(`<img src="${url}" />`, "请使用微信支付", {
        ...
        showClose: false,
        // action 的值为'confirm'或'cancel'；instance 为 MessageBox 实例，可以通过它访问实例上的属性和方法；done 用于关闭 MessageBox 实例
      ...
      // 支付成功路由跳转，支付失败提示信息 reqPaymentStatus
      if (!this.timer) {
        this.timer = setInterval(async () => {
          let res = await this.$API.reqPaymentStatus(this.orderId);
          if (res.code === 200) {
            clearInterval(this.timer);
            this.timer = null;
            // 保存支付成功的code
